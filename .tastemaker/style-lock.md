...
- Logo: `design/assets/logo/yoldr-mark.svg` (transparent, in-app use — nav/sidebar on black backgrounds) + `design/assets/logo/yoldr-lockup.svg` (mark + Playfair-italic wordmark) + `design/assets/favicons/yoldr-mark-favicon.svg` (mark on a black rounded-square tile, for browser-chrome legibility on any theme). Source: **constructed (Path A)** — original geometric shapes, no licensing question. Concept: a shield silhouette (white) — chosen because "Shields" is literally Yoldr's investment-product name, not a generic security cliché — with a single accent-orange dot breaking from its top-right shoulder on an implied arc, encoding the brand's own tagline: *your principal stays home (the shield), your yield goes adventuring (the escaping dot)*. 2 shapes, describable in one sentence, tested at 16px.
